...
$(document).ready(function(){
  ...
  $('#datagridExample').datagrid({
    dataSource: {
        cols:[
            {name: 'id', label: '订单id', width: 132},
            {name: 'userId', label: '用户ID', width: 134},
            {name: 'userName', label: '用户姓名', width: 109},
            {name: 'userTel', label: '用户电话', width: 109},
            {name: 'courseId', label: '商品编号', width: 287},
            {name: 'courseName', label: '商品名称', width: 287},
            {name: 'busId', label: '商家ID', width: 287},
            {name: 'busName', label: '商家名称', width: 287},
            {name: 'reserveNum', label: '预约人数', width: 287},
            {name: 'price', label: '价格', width: 287},
            {name: 'orderState', label: '订单状态', width: 287},
            {name: 'userNote', label: '用户备注', width: 287},
            {name: 'orderCreateTime', label: '订单创建时间', width: 287}
        ],
        array:[
        	<c:forEach items="${list}" var="list">
    		{ id:"${list.id}",userId: "${list.userId}", userName: "${list.userName}", userTel:"${list.userTel}",courseId:"${list.courseId}",courseName:"${list.courseName}",busId:"${list.busId}",busName:"${list.busName}",reserveNum:"${list.reserveNum}",price:"${list.price}",orderState:"${list.orderState}",userNote:"${list.userNote}",orderCreateTime:"${list.orderCreateTime}"},   		
    		</c:forEach>
        ]
    },
    //特殊列的参数
...
function al(){
	...
	var xmlhttp;
	 ...
	 xmlhttp.open("POST","${ctx }/bg_reservation/bgLockRe",true);
...
<div id="content">
  ...
  <button type="button" onclick="al()" class="btn">锁定订单</button>
