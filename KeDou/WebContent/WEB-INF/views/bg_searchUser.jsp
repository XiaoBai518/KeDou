...
$(document).ready(function(){
  ...
  //回传数据 注意数据JSTL不能和HTML中参数混用
  $('#datagridExample').datagrid({
    dataSource: {
        cols:[
            {name: 'userId', label: '用户ID', width: 132},
            {name: 'userName', label: '用户昵称', width: 134},
            {name: 'userEmail', label: '用户邮箱', width: 109},
            {name: 'usrTel', label: '用户电话', width: 109},
            {name: 'userIp', label: '用户IP', width: 287}
        ],
        array:[
           
        		<c:forEach items="${list}" var="list">
        		{ userId:"${list.userId}",userName: "${list.userName}", userEmail: "${list.userEmail}", userTel:"${list.userTel}",userIp:"${list.userIp}"},   		
        		</c:forEach>
...
function al(){
	...
	var xmlhttp;
	 ...
	 xmlhttp.open("POST","${ctx }/bg_admin/bgLockUser",true);
...
<div id="content">
  ...
  <button type="button" onclick="al()" class="btn">锁定</button>
